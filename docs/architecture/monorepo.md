...
A implementação usa `apps/frontend` (equivalente ao `apps/web` planejado) e `compose.yml` na raiz. Dockerfiles ficam junto de cada aplicação. Gateway/Auth/Customer foram entregues na Fase 1, Catalog na [Fase 2](../catalog.md) e Order começou pelo carrinho na [Fase 3](../cart.md). A árvore abaixo representa o alvo completo, não serviços já implementados.
