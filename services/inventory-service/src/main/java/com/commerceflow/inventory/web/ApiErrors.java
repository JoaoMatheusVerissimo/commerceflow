...
public class ApiErrors extends OncePerRequestFilter {
    ...
    @Override protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        String id;
        try { id = UUID.fromString(request.getHeader("X-Correlation-Id")).toString(); }
        catch (RuntimeException ex) { id = UUID.randomUUID().toString(); }
        request.setAttribute("correlationId", id); response.setHeader("X-Correlation-Id", id);
